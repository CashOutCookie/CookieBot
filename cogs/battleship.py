import discord, re, asyncio, logging, random, typing
...
class Game:
    ...
    async def take_turn(self) -> typing.Optional[Square]:
        ...
        turn_message = await self.turn.user.send(
            "It's your turn! Type the square you want to fire at, first the letter then number, like this: `A1`\nType **surrender** to surrender yourself."
        )
    ...
    async def hit(self, square: Square, alert_messages: typing.List[discord.Message]) -> None:
        await self.turn.user.send("Hit!", delete_after=2.0)
...
class BattleShips(commands.Cog):
    ...
    async def battleship(self, ctx: commands.Context) -> None:
        ...
        announcement = await ctx.send(
            ...
            f"Press {ACCEPT} to play against {ctx.author.mention}!\n"
